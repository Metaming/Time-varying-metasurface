%%% this is a program that use simulated scattering parameters of a metaunit in the waveguide to generate sideband signals by mimiking a time-varying modulation. Genetic algorithm is used to optimze the modulation signal
...
s21fin1=fopen('s21_ESRR+SRR2_varactor2.txt');
s21phfin1=fopen('s21phase_ESRR+SRR2_varactor2.txt');
s11fin1=fopen('s11_ESRR+SRR2_varactor2.txt');
s11phfin1=fopen('s11phase_ESRR+SRR2_varactor2.txt');

%s21fin1=fopen('s21_ESRR+SRR2_varactor_period.txt');
%s21phfin1=fopen('s21phase_ESRR+SRR2_varactor_period.txt');
%s11fin1=fopen('s11_ESRR+SRR2_varactor_period.txt');
%s11phfin1=fopen('s11phase_ESRR+SRR2_varactor_period.txt');
...
vv1=2;
vv2=1;
[dd,nv1]=min(abs(vv1-V1));
[dd,nv2]=min(abs(vv2-V2));

S21_vv=S21_cc(:,nv1,nv2);
S11_vv=S11_cc(:,nv1,nv2);

Z_E=377/2.*(1+(S11_vv+S21_vv))./(1-(S11_vv+S21_vv));
Z_M=377*2.*(1+(S11_vv-S21_vv))./(1-(S11_vv-S21_vv));


figure(111);
plotyy(F, abs(S21_vv), F,angle(S21_vv));hold off
%plot(F, abs(S21_vv));hold off


figure(21);

subplot(1,2,1)
plot(F, real(Z_E));hold on;
plot(F, imag(Z_E));hold off;
legend('Re(Z_E)','Im(Z_E)')
xlabel('Freq (GHz)'); ylabel('Z_E (Ohm)')

subplot(1,2,2)
plot(F, real(Z_M));hold on;
plot(F, imag(Z_M));hold off;
legend('Re(Z_M)','Im(Z_M)')
xlabel('Freq (GHz)'); ylabel('Z_M (Ohm)')
...
figure(201)
subplot(1,3,1)
sf=contour(V1I,V2I,angle(s11_CI),15);
%set(sf, 'EdgeColor', 'none');
xlabel('V1(V)');
ylabel('V2(V)');
axis([min(V1),max(V1),min(V2),max(V2)])
...
subplot(1,3,2)
sf=contour(V1I,V2I,angle(s21_CI),15);
%set(sf, 'EdgeColor', 'none');
xlabel('V1(V)');
ylabel('V2(V)');
axis([min(V1),max(V1),min(V2),max(V2)])
...
Om=0.5e6;    %%%% Hz, frequency of modulation
t=-1.0/Om:1e-3/Om:1.0/Om;
V1_0=1;  %%% center of biased voltage
V2_0=2;

deltaV1=1.5;deltaV2=1.5; %%% amplitude of biased voltage
    ...
    offs2_0=offs2.*(1+random('unif',-mutof,mutof,1,1))+random('unif',-mutof,mutof,1,1);
             ...
             %%%%% Geno
                for n_geno=1:1:N_Geno
                  ...
                  %%% for geno number larger than 5, introduce mutation
                      ...
                      % better
                        if am1+offs1>9 am1=9-offs1;
                        end
                        if -am1+offs1<-0.8 am1=offs1+0.8;
                        end
                        if am2+offs2>9 am2=9-offs2;
                        end
                        if -am2+offs2<-0.8 am2=offs2+0.8;                            
                        end
                ...
                OBJ11(n_geno)=abs(As21_linear(n_objhm))./abs(As11_linear(n_objhm+2));
                OBJ21(n_geno)=(abs(As21_linear(n_objhm))^2+1.*abs(As11_linear(n_objhm+2))^2)./(sum(abs(As21_linear).^2+abs(As11_linear).^2)); 
        ...
        % draw the best waveform and spectrum
            ...
            for n=1:n_od 
                ...
                signal1_best=(sign1_best);
                signal2_best=(sign2_best);
                 end
...
for n_dph=1:length(dphi)
  ...
  for pn=1:n_round
        ...
        for n=1:n_od 
            ...
            if iden_waveform==1
               A2_best_allpass(pn,n)=A1_best_allpass(pn,n); B2_best_allpass(pn,n)=B1_best_allpass(pn,n);PH2_best_allpass(pn)=PH2_best_allpass(pn);
  ...
  % total signal
              ...
              if n_od>1
                ...
                signal2_best_allpass=sum(sign2_od);
                 ...
                 signal1_best_allpass=(sign1_od);
                signal2_best_allpass=(sign2_od); 
              end
                % normalized signal
...
%
s21_t=s21_best_allpass_t(nbest,:);
s11_t=s11_best_allpass_t(nbest,:);
...
nz_od=40; % number of Fourier order of Z to calculate
N_order=-nz_od:1:nz_od;

Z_E_a=zeros(1,nz_od+1);
Z_E_b=zeros(1,nz_od+1);
Z_M_a=zeros(1,nz_od+1);
Z_M_b=zeros(1,nz_od+1);
...
Z_E_a(n+1)=1/pi.*sum(Z_E.*fa)*dx;
Z_E_b(n+1)=1/pi.*sum(Z_E.*fb)*dx;

Z_M_a(n+1)=1/pi.*sum(Z_M.*fa)*dx;
Z_M_b(n+1)=1/pi.*sum(Z_M.*fb)*dx;
end

Z_E_p=0.5.*(Z_E_a-1i.*Z_E_b);
Z_E_n=0.5.*(Z_E_a+1i.*Z_E_b);
Z_M_p=0.5.*(Z_M_a-1i.*Z_M_b);
Z_M_n=0.5.*(Z_M_a+1i.*Z_M_b);

Z_En=[fliplr(Z_E_n(2:end)),Z_E_p];
Z_Mn=[fliplr(Z_M_n(2:end)),Z_M_p];


%%
nz_sb=6;  %%% number of sideband to consider
N_har=-nz_sb:1:nz_sb;

Z_Et=zeros(2*nz_sb+1,2*nz_sb+1);
Z_Mt=zeros(2*nz_sb+1,2*nz_sb+1);
...
for n=1:2*nz_sb+1
   ...
   for m=1:2*nz_sb+1
       ...
       n_c=find(n-m==N_order);
       if isempty(n_c)==0
       
       Z_Et(m,n)=Z_En(n_c);
       Z_Mt(m,n)=Z_Mn(n_c);
       
       else Z_Et(m,n)=0;Z_Mt(m,n)=0;
       end
...
V_E=zeros(2*nz_sb+1,1);V_E(nz_sb+1)=1;
V_M=zeros(2*nz_sb+1,1);V_M(nz_sb+1)=1;
...
As21_z_n=abs(As21_z).^2./sum( (abs(As21_z).^2+abs(As11_z).^2));
As11_z_n=abs(As11_z).^2./sum( (abs(As21_z).^2+abs(As11_z).^2));

figure(101)
stem(N,abs(AS21_best),'*');hold on;
stem(N,-abs(AS11_best),'*');hold on;
stem(-nz_sb:1:nz_sb,abs(As21_z_n),'o');hold on;
stem(-nz_sb:1:nz_sb,-abs(As11_z_n),'o');hold off;
axis([-10.5,10.5,-0.50,0.5])


figure(12)
subplot(1,2,1)
semilogy(-nz_sb:1:nz_sb,abs(As21_z_n),'ko');hold on;
semilogy(-nz_sb:1:nz_sb,abs(As11_z_n),'bo');hold on;
semilogy(N,AS21_best,'k*');hold on;
semilogy(N,AS11_best,'b*');hold off;
